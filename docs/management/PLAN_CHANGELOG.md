...
- **メモ履歴ページ新規作成**
  ...
  - 顧客ごとのメモ・記録を一覧表示
  - フィルター機能: すべて / メモ / 記録
  - 全文表示（折りたたみなし）で流し読みに最適化

- **プリセット設定の機能強化**
  - 各プリセットの下に「プロンプトを見る」リンク追加
  - モーダルでプロンプト全文表示
  - コピー・ダウンロード機能

- **AI整形モーダルのUX改善**
  - 「💡 適用後、入力欄で内容を編集できます」ヒント追加
  - 適用→編集のフローが明確に

- **VoiceLog機能の廃止**
  - AI整形結果の別途保存をやめ、メモ履歴で代替
  - 関連ファイル削除: `voiceLogActions.ts`, `/voice-logs` ページ等
  - DBモデル（VoiceLog）はスキーマに残存（将来の拡張余地）

- **顧客詳細ページのリファクタ**
  - 「AI整形履歴」→「メモ履歴」リンクに変更
  - プロフィールタブからメモ履歴ページへ遷移可能

### 変更の背景
- ユーザーからのフィードバック：顧客詳細ページでAI整形履歴がレイアウト崩れの原因に
- シンプルなメモ一覧の方が業務フローに適していると判断
- プロンプト確認機能は開発者・管理者が設定内容を把握するために追加
...
- **デモ用サンプルデータの修正**
  ...
  - 17:30以降の予約データが確実に生成されるよう修正

- **音声フィルタの競合問題修正**
  - 新しいフィルタ条件（「17時以降」等）適用時に、古い競合条件（「午前」等）を自動クリアするよう修正
  - 連続した音声コマンドでフィルタが機能しなくなる問題を解消

- **申し送りフィルタの修正**
  - バッジカウントとフィルタ結果が一致するよう修正
  - 解決済み・未解決に関わらず、申し送りがある全予約を表示するよう変更

- **モバイルスクロール問題の修正**
  - 予約ページ (`ReservationV2Client.tsx`): `overflow-y-scroll` に変更
  - 顧客詳細ページ (`CustomerDetailClient.tsx`): `h-full overflow-y-auto overflow-x-hidden` を追加

- **UI改善**
  - 当日以外の予約カードに日付を表示するよう変更
