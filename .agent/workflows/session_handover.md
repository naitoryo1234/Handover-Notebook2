...
7. **引き継ぎメッセージの生成 (Generate Handover Message)**:
   ...
   - 出力フォーマット例:
     ...
     > 1. `docs/ai-host/INIT_ROUTER.md` を読み込んでください。
     ...
     >
     > **ユーザーの目的 (USER OBJECTIVE)**: 初期化を行い、`docs/management/PLAN_CHANGELOG.md` 等に記載された「次回のタスク」候補の中から、ユーザーに着手するタスクを確認・提案してください。
     >
